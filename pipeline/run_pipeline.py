...
from modelscope import AutoTokenizer, AutoProcessor, AutoModelForCausalLM
...
def generate_and_save_completions_for_dataset(cfg, model_base, processor, fwd_pre_hooks, fwd_hooks, output_file, dataset, batch_size):
    ...
    # completions = generate_completions(model_base, processor, fwd_pre_hooks=fwd_pre_hooks, fwd_hooks=fwd_hooks, dataset=dataset, batch_size=batch_size, max_new_tokens=cfg.max_new_tokens)
    ...
    logits_output_file = output_file.replace('.jsonl', '_logits.json')
...
def run_pipeline(model_path, train_obv, train_obv_nh, train_elu, train_elu_nh, val_obv, val_elu, val_nh, test_obv, test_elu, test_nh):
    ...
    model_base = Qwen2_5_VLForConditionalGeneration.from_pretrained(model_path, torch_dtype="auto", device_map="auto").eval()               # Qwen2.5-VL
    # model_base = AutoModelForCausalLM.from_pretrained(model_path, torch_dtype="auto", device_map="auto", trust_remote_code=True).eval()   # LLaVA-OneVision-1.5-8B
    
    model_base.requires_grad_(False)

    processor = AutoProcessor.from_pretrained(model_path)                               # Qwen2.5-VL
    # processor = AutoProcessor.from_pretrained(model_path, trust_remote_code=True)     # LLaVA-OneVision-1.5-8B
    ...
    for i in range(1, 15, 1):
        ...
        dual_ablation_fwd_pre_hooks, dual_ablation_fwd_hooks = get_all_multi_direction_ablation_hooks(
            model_base, directions=[direction_obv, direction_elu],
            coeffs=[obv_coeff, elu_coeff]
        )

        output_file_obv = os.path.join(cfg.artifact_path(), f"test_obvious.jsonl")
        ...
        output_file_elu = os.path.join(cfg.artifact_path(), f"test_elusive.jsonl")
        ...
        output_file_nh = os.path.join(cfg.artifact_path(), "test_nh_elusive.jsonl")
        ...
        metrics_log_path = os.path.join(cfg.artifact_path(), "dual_ablation_metrics.jsonl")
        obv_metrics_path = output_file_obv.replace(".jsonl", "_logits.json")
        elu_metrics_path = output_file_elu.replace(".jsonl", "_logits.json")
        nh_metrics_path = output_file_nh.replace(".jsonl", "_logits.json")

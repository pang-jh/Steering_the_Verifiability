...
def nh_label(item):
    ...
    if item.get("gt") == "否" and pred.startswith("否"):
        return "no_no"
...
def count_nh_binary(items):
    no_no = sum(1 for x in items if nh_label(x) == "no_no")
...
def collect_nh_balanced_by_images(groups, images, rng):
    ...
    k = min(len(no_no), len(yes_yes))
    if k == 0:
        return []

    out = rng.sample(no_no, k) + rng.sample(yes_yes, k)
    rng.shuffle(out)
    return out

def main():
    obvious_path = "/dataset/all_data/7b/filter/obvious_filter.jsonl"
    elusive_path = "/dataset/all_data/7b/filter/elusive_filter.jsonl"
    nh_path = "/dataset/all_data/7b/filter/nh_filter.jsonl"

    out_train_obvious = "/dataset/all_data/7b/filter/pure_balance/train_obvious.jsonl"
    out_train_elusive = "/dataset/all_data/7b/filter/pure_balance/train_elusive.jsonl"
    out_train_nh = "/dataset/all_data/7b/filter/pure_balance/train_nh_full.jsonl"

    out_val_obvious = "/dataset/all_data/7b/filter/pure_balance/val_obvious.jsonl"
    out_val_elusive = "/dataset/all_data/7b/filter/pure_balance/val_elusive.jsonl"
    out_val_nh = "/dataset/all_data/7b/filter/pure_balance/val_nh_full.jsonl"

    out_test_obvious = "/dataset/all_data/7b/filter/pure_balance/test_obvious.jsonl"
    out_test_elusive = "/dataset/all_data/7b/filter/pure_balance/test_elusive.jsonl"
    out_test_nh = "/dataset/all_data/7b/filter/pure_balance/test_nh_full.jsonl"
    ...
    tr_no_no, tr_yes_yes = count_nh_binary(train_nh_items)
    v_no_no, v_yes_yes = count_nh_binary(val_nh_items)
    t_no_no, t_yes_yes = count_nh_binary(test_nh_items)
